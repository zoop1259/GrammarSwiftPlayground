...
//(1)의 경우 Int 타입의 빈 배열을 arr 변수에 할당합니다. 이 구문은 arr 변수에 Int 타입의 빈 배열을 할당하는 구문입니다.
//유형 주석을 사용하여 배열 유형을 정수 배열로 명시적으로 지정합니다.
//(2)의 경우 새로운 Int 타입의 빈 배열을 생성하고, arr 변수에 할당합니다. 이 구문은 새로운 Int 타입의 빈 배열을 생성하는 초기화 구문입니다.
//Swift의 유형 추론을 사용 arr하여 배열에 추가되는 객체의 유형을 기반으로 정수 배열을 추론합니다.
//(1)
let emptyArray: [Int] = []
